...
- Тригери: (1) при вході користувача в систему; (2) heartbeat кожні 5 хв — піднімає бот, якщо помер.
- Запускає бот у прихованому вікні, логи пише в `data\bot.log`.
- `MultipleInstances IgnoreNew` — повторні запуски пропускаються, поки процес живий.
- Sleep / hibernate ноута → бот спить разом з ним; після прокидання — heartbeat підніме його за <5 хв.
